...
def build_ai_notes(root, pkgs, md, fm):
    ...
    m = md["measured"]
    ap = pkgs.get("aplus", {})
    hero_manifest_p = os.path.join(root, "07_ASSETS", "GBK02_GAME_ILLUSTRATION_MANIFEST.json")
    hero_manifest = load(hero_manifest_p) if os.path.exists(hero_manifest_p) else {}
...
| | |
|---|---|
| Produced by | one dedicated illustration per game, {hero_manifest.get('totalImages', 0)} of {hero_manifest.get('totalGames', 0)} games ({hero_manifest.get('coverage', 'n/a')}) |
| Tool | {hero_manifest.get('generationTool', 'GPT Image')} |
...
- **Images — AI-generated:** yes — the cover, the A+ artwork, and every
  interior hero illustration ({hero_manifest.get('totalImages', 0)} of them); **no** for the
...
images, and the {hero_manifest.get('totalImages', 0)} interior hero illustrations are AI-generated. The
